...
skip_unixtime:         
    ...
    //DiscardData(); // This resets the ImageInfo struct. Line commented by E.Rodolà
    return;
badtime:
...
//--------------------------------------------------------------------------
// Authors: Emanuele Rodolà, Andrea Albarelli
//--------------------------------------------------------------------------
int extract_focal_length(const char* filename, float* focal_length, float ccd_width) {

camera_ccd cameras_ccd[] = {
    ...
    {"Canon Canon PowerShot SX100 IS",5.72},
   ...
   *focal_length = ImageInfo.FocalLength;

   if (ImageInfo.Width <= 0) {
      fprintf(stderr, "Error. EXIF width not found\n");
      return FOCAL_LENGTH_INVALID_EXIF;
   }

   if (ccd_width == 0.) {
      char buf[1000];
      bzero(buf,1000);
      strcat(buf, ImageInfo.CameraMake);
      strcat(buf + strlen(ImageInfo.CameraMake), " ");
      strcat(buf + strlen(ImageInfo.CameraMake) +1, ImageInfo.CameraModel);

      camera_ccd* ptr = cameras_ccd;
      while(ptr->ccd_width != 0) {
         if (strcmp(ptr->model,buf) == 0) {
            ImageInfo.CCDWidth = ptr->ccd_width;
            break;
         }
         ++ptr;
      }

      if (ImageInfo.CCDWidth <= 0) {
         fprintf(stderr,"Error. EXIF CCD data found.\n");
         return FOCAL_LENGTH_NO_CCD_DATA;
      }
   }

   else { // ccd width given
      ImageInfo.CCDWidth = ccd_width;
   }

   *focal_length = ImageInfo.Width * (*focal_length) / ImageInfo.CCDWidth;

   return FOCAL_LENGTH_OK;
}

int main()
{
   float focal_len, ccd_width = 0.;
   extract_focal_length("foto/IMG_0113.JPG", &focal_len, ccd_width);
   fprintf(stdout, "focal length: %f\n", focal_len);
