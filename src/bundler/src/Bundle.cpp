...
{
#define MIN_POINTS 16 //FIXME: prima era 20
    int num_outliers = 0;
    ...
    do {
        if (num_pts - total_outliers < MIN_POINTS) {
            printf("[RunSFM] Too few points (%d < %d) remaining, exiting!\n", num_pts - total_outliers, MIN_POINTS);
            fflush(stdout);
...
#define NUM_STDDEV 2.0 // 3.0 // 6.0
            ...
            thresh = CLAMP(thresh, m_min_proj_error_threshold, m_max_proj_error_threshold);
            ...
            printf("[RunSFM] Mean error cam %d[%d] [%d pts]: %0.3e "
                "[med: %0.3e, outl thresh: %0.3e]\n",
                i, added_order[i], num_pts_proj, avg, 
                kth_element_copy(num_pts_proj, iround(0.5 * num_pts_proj), dists),
                thresh);
...
#define NUM_ERROR_BINS 10
            ...
            /* Break histogram into NUM_ERROR_BINS bins */
            int idx_count = 0;
            printf("  error histogram follows:\n");
